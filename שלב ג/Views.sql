...
-- This view combines TRIP, GUIDE, and ROUTE to provide a detailed look at planned trips.
-- ------------------------------------------------------
...
-- ------------------------------------------------------
-- VIEW 2: Perspective of the new department (Financials/Payments & Bookings status)
-- This view focuses on BOOKING, PAYMENT, and PAYMENTSTATUS (from the integrated side).
-- ------------------------------------------------------
...
GROUP BY 
    b.BookingID, b.BookingDate, p.FullName, p.Email, b.AmountToPay, rs.StatusName;

-- Query 2.1: Find all customers with outstanding balances (AmountToPay > TotalPaid)
-- Description: Identifies which bookings are not fully paid yet, so they can be reminded.
...
-- Query 2.2: Summarize total revenue received vs expected by booking status
-- Description: High-level financial overview grouped by booking status.
