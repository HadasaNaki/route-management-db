...
# Routes DB: Guided Tours System

**Project Information**
* Prepared by: Hadassa Niki, Tehila Cohen
* System Name: Routes DB
* Module: Tours Booking, Registration
...
![תרשים יחסי המערכת](./שלב%20א/ERDandDSDfiles/erdplus.png)

*תרשים 2: תרשים מבנה נתונים (ERD)*
![סכמות מערכת](./שלב%20א/ERDandDSDfiles/erdplus%20%281%29.png)
...
* [הכנסת נתונים (Insert Data)](./init-db/03_insertTables.sql) – סקריפט המכיל נתונים ראשוניים.
* [בדיקת טבלאות (Select All)](./שלב%20א/scripts/selectAll.sql) – שאילתות בדיקה לספירת רשומות ושליפה לדוגמה מכל טבלה.
...
* **איך זה עובד אצלנו?** הסקריפט הנפרד `bulk_generate.py` משתמש במחולל `Faker`. במקום לייבא קבצים ידנית, הוא מגריל מאות שמות, טלפונים נוכחיים, תאריכים ומחירים, ומזריק אותם בבת אחת למערכת הפוסטגרס. הסקריפט בורא מאות מדריכים חדשים, ומעל ל- **20,000 רשומות רנדומליות** של משתתפים והזמנות.

#### הצגת הנתונים בטבלאות ממערכת pgAdmin

**טבלת משתתפים (Participant) - מעל 20,000 רשומות**

![צילום מסך טבלת משתתפים](./שלב%20א/ERDandDSDfiles/countparticipant.png)
![צילום מסך טבלת משתתפים](./שלב%20א/ERDandDSDfiles/Participant.png)

**טבלת טיולים (Trip)**

![צילום מסך טבלת טיולים](./שלב%20א/ERDandDSDfiles/trip.png)
...
![צילום מסך גיבוי](./שלב%20א/backup/backup1.png)
![צילום מסך גיבוי](./שלב%20א/backup/backup2.png)
...
![צילום מסך שחזור](./שלב%20א/backup/restore.png)
...
```sql
SELECT FullName, Phone FROM PARTICIPANT
WHERE ParticipantID IN (
  SELECT ParticipantID FROM BOOKING WHERE TripID IN (
    SELECT TripID FROM TRIP WHERE RouteID IN (
      SELECT RouteID FROM PASSES_THROUGH WHERE LocationID IN (
        SELECT LocationID FROM LOCATION WHERE Category IN ('Historic', 'Nature')
      )
    )
  )
);
...
```sql
SELECT p.FullName, p.Phone FROM PARTICIPANT p
WHERE EXISTS (
  SELECT 1 FROM BOOKING b
  JOIN TRIP t ON b.TripID = t.TripID
  JOIN PASSES_THROUGH pt ON t.RouteID = pt.RouteID
  JOIN LOCATION l ON pt.LocationID = l.LocationID
  WHERE b.ParticipantID = p.ParticipantID AND l.Category IN ('Historic', 'Nature')
);
```
...
FROM ROUTE r JOIN TRIP t ON r.RouteID = t.RouteID
...
```sql
SELECT RouteName, TotalTrips FROM (
  SELECT r.RouteName,
    (SELECT COUNT(*) FROM TRIP t WHERE t.RouteID = r.RouteID
     AND EXTRACT(YEAR FROM t.DepartureDate) = 2026) AS TotalTrips
  FROM ROUTE r
) AS Temp
WHERE TotalTrips > 0;
```
...
```sql
SELECT FirstName, LastName FROM GUIDE
...
```sql
SELECT g.FirstName, g.LastName FROM GUIDE g
...
```sql
SELECT t.TripID, r.Difficulty, t.Price FROM TRIP t
JOIN ROUTE r ON t.RouteID = r.RouteID
WHERE t.Price = (
  SELECT MAX(t2.Price) FROM TRIP t2
...
```sql
SELECT TripID, Difficulty, Price FROM (
...
WHERE b.Status IN ('Paid', 'Confirmed') AND EXTRACT(YEAR FROM t.DepartureDate) = 2026
...
```sql
SELECT p.FullName, p.Email, p.Phone FROM PARTICIPANT p
JOIN BOOKING b ON p.ParticipantID = b.ParticipantID
JOIN TRIP t ON b.TripID = t.TripID
JOIN ROUTE r ON t.RouteID = r.RouteID
WHERE r.Difficulty = 'Hard'
AND r.RouteID IN (
  SELECT RouteID FROM PASSES_THROUGH GROUP BY RouteID HAVING COUNT(LocationID) > 0
);
```
...
```sql
SELECT t.TripID, r.RouteName, (g.FirstName || ' ' || g.LastName) AS GuideName,
  t.DepartureDate, t.MaxCapacity
FROM TRIP t
JOIN ROUTE r ON t.RouteID = r.RouteID
...
```sql
UPDATE TRIP SET Price = Price * 1.10
WHERE RouteID IN (
...
```sql
UPDATE GUIDE SET Expertise = 'Senior ' || Expertise
WHERE GuideID IN (
  SELECT GuideID FROM TRIP GROUP BY GuideID HAVING COUNT(TripID) > 5
)
...
AND TripID IN (
  SELECT TripID FROM TRIP WHERE DepartureDate <= CURRENT_DATE + INTERVAL '3 days'
);
```
...
WHERE LocationID NOT IN (SELECT LocationID FROM PASSES_THROUGH);
```
...
```sql
INSERT INTO TRIP (TripID, DepartureDate, MaxCapacity, Price, RouteID, GuideID)
VALUES (901, '2026-06-01', 20, -50, 101, 1);
```
...
```sql
INSERT INTO BOOKING (BookingID, BookingDate, Status, TripID, ParticipantID)
VALUES (901, '2026-05-19', 'UnknownStatus', 1001, 1);
```
...
```sql
INSERT INTO ROUTE (RouteID, RouteName, Duration, Difficulty)
VALUES (901, 'Bad Route', 100, 'Extreme');
```
...
**שלב 1 – הצגת מחיר התחלתי:**

```sql
SELECT Price FROM TRIP WHERE TripID = 1001;
```

![לפני הרולבק](./image/ו%20לפני%20.png)

**שלב 2 – ביצוע שינוי בתוך טרנזקציה:**
